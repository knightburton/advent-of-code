...
program
  ...
  .action(day => {
    ...
    fs.readFile(path.resolve(__dirname, `./day-${DDay}/input.txt`), (err, puzzle) => {
      ...
      console.log(`Day ${DDay} puzzle first part solution: ${puzzleSolver.default.first(puzzle.toString())}`);
      console.log(`Day ${DDay} puzzle second part solution: ${puzzleSolver.default.second(puzzle.toString())}`);
    });
  })
  .parse(process.argv);
